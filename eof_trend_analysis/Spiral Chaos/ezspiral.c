...
static  FILE *history_file, *tip_file, *hist_file_v; 
...
static void  Write_history_v (int wrt_step);
...
void Initialize (void)
{
  ...
  Step_ini();
  if(hist_step){
	  history_file = fopen("hist_big3.dat", "w");
	  /*hist_file_v  = fopen("histor_v.dat", "w");*/
  }
...
void Write_history (int wrt_step){
   ...
   /*for (j = 1; j < NY; j++) {
	...
	for(j=1;j<=NY;j++) {
		for(i=1;i<=NX;i++) {
			fprintf(history_file, "%.5f %d %d %g\n", dt*(Real)wrt_step, 
			i, j, (float)U(i,j)); 
		}
	} 
}

void Write_history_v (int wrt_step){
	
	int i, j;
	for(j=1;j<=NY;j++) {
		for(i=1;i<=NX;i++) {
			fprintf(hist_file_v, "%.5f %d %d %g\n", dt*(Real)wrt_step, 
			i, j, (float)V(i,j)); 
		}
	} 
}
